...
<div id="wrapper">
    ...
    <nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
        ...
        <ul class="nav navbar-top-links navbar-right">
            ...
            <!-- /.dropdown -->
            <li class="dropdown">
                <a class="dropdown-toggle" data-toggle="dropdown" href="#" id="alert_click">
                    <i class="fa fa-bell fa-fw"></i> <b id="notification_count">(0)</b> <i class="fa fa-caret-down"></i>
                </a>
                <!-- /.dropdown-alerts -->
                <ul class="dropdown-menu dropdown-alerts" id="alert_list">
...
<script>
    $(document).ready(function(){
        $.get(baseUrl + '/admin/ajax', function(response){
            if (response.success != undefined && response.success === true) {
                $('#notification_count').text('(' + response.count_new + ')');
                $('#alert_list').html(response.html);
            }
        });
        ...
        $('#alert_click').click(function(){
            var listRead = [];
            $('#alert_list > li').each(function(){
                if ($(this).attr('attr-id')) {
                    listRead.push($(this).attr('attr-id'));
                }
            });

            if (listRead.length > 0) {
                $.post(baseUrl + '/admin/notify', { listRead :  listRead}, function(response){
                    if (response.success != undefined && response.success === true) {
                        $('#notification_count').text('(' + response.count_new + ')');
                    }
                });
